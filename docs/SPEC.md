...
| Repair exhaustion diagnostics | Record the blocker reason for every unresolved quota, credit, package, or off-plan failure after repair. | Makes failed-quality actionable instead of merely saying repair ran. | Responses distinguish no candidate, no safe removal, candidate-pool cap, pool limit, role-count semantics, and iteration-budget exhaustion. |
| Effective quota semantics | Evaluate assigned count and effective credit separately for infrastructure roles. | Prevents export-blocking failures for one-slot infrastructure count gaps that are already covered by strong credit. | RAMP/CARD_DRAW/SPOT_REMOVAL/PROTECTION may pass a one-slot count deficit only when credit meets target and coherence passes; win conditions and archetype roles remain hard count gates. |
| Mana-fixing land classification | Classify primary color-fixing lands separately from true utility lands. | Prevents premium fixing lands from being rejected as low-relevance utility cards. | Fetch/shock/check/pain/filter/command-style fixing lands are ranked by mana-base quality, not utility synergy; warning spam is avoided. |
...
| Repair blocker diagnostics | Generate failed-quality decks with no candidates, no removable cards, candidate cap exclusions, and max-iteration exhaustion. | Each remaining failure has a specific blocker reason. | P0 | Recommendation/API/UI |
| Effective quota count | Generate decks with infrastructure count one below target but credit above target. | Infrastructure gaps can be credit-covered only when coherence passes; win-condition and archetype gaps remain hard failures. | P0 | Recommendation/API/UI |
| Mana-fixing land classification | Generate multicolor decks with premium fixing and tapped low-quality duals. | Premium fixing is not rejected as low-relevance utility and mana-base cuts are summarized cleanly. | P0 | Recommendation/QA |
...
| 2026-05-18 | Added post-wave-12 requirements for repair blocker diagnostics, effective quota count semantics, and separating mana-fixing lands from utility-land relevance filtering. |
| 2026-05-18 | Implemented mana-fixing land classification: primary fixing lands bypass utility-land relevance filtering, keep MANA_FIXING tags, and mono-color fixing exclusions remain intact. |
| 2026-05-18 | Implemented effective quota semantics: RAMP, CARD_DRAW, SPOT_REMOVAL, and PROTECTION can expose credit-covered one-slot count gaps when coherence passes, while win-condition and archetype-role deficits remain hard failures. |
