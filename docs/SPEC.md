...
| Deck selection | Greedy role-fill + local swaps or ILP for later version. | Simpler MVP; can improve over time. | Final deck passes legality and role validation. |
| Validation | Run legality, singleton, card count, curve, role quota, and duplicate checks. | Prevents embarrassing bad outputs. | No generated deck can be exported until validation passes. |
